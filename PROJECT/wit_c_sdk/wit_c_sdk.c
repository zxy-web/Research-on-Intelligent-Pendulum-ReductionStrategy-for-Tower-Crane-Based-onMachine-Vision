...
static uint8_t __CaliSum(uint8_t *data, uint32_t len)//串口接收到数据进行校验和计算
{
    uint32_t i;
    uint8_t ucCheck = 0;
    for(i=0; i<len; i++) ucCheck += *(data + i);//for循环，根据长度len进行计算，(data+i)是地址进行加i，但是括号前面加了*号就是该地址对应的数据
    return ucCheck;//返回校验和ucCheck
}
int32_t WitSerialWriteRegister(SerialWrite Write_func)//注册回调函数
{
    if(!Write_func)return WIT_HAL_INVAL;//判断Write_func是否有数据传入，有的话就跳过return
    p_WitSerialWriteFunc = Write_func;//把断Write_func数据赋值到p_WitSerialWriteFunc
    return WIT_HAL_OK;//返回WIT_HAL_OK
}
static void CopeWitData(uint8_t ucIndex, uint16_t *p_data, uint32_t uiLen)//处理接收到的数据
{
    ...
    uiReg1Len = 4;
    switch(ucIndex)//ucIndex是接收到的第二个数据，查询下面的case对应的就是0x50这些
    {
        ...
        case WIT_TIME:  uiReg1 = YYMM;	break;//根据ucIndex接收到的不同进行不同的操作
    ...
    if(uiLen == 3)//在接收到的数据是WIT_GYRO、WIT_MAGNETIC这几个时才会执行括号里面的操作
    {
        uiReg1Len = 3;
        uiReg2Len = 0;
    }
    if(uiReg1Len)//在接收到的数据是WIT_ACC、WIT_ANGLE、WIT_GYRO、WIT_MAGNETIC这几个时才会执行括号里面的操作
	{
		memcpy(&sReg[uiReg1], p_usReg1Val, uiReg1Len<<1);//底层函数数组数据前移，如：s_ucWitDataBuff[1]的数据移动到s_ucWitDataBuff[0]的位置
		p_WitRegUpdateCbFunc(uiReg1, uiReg1Len);
	}
    if(uiReg2Len)//在接收到的数据是WIT_ACC，WIT_ANGLE会执行括号里面的操作
	{
		memcpy(&sReg[uiReg2], p_usReg2Val, uiReg2Len<<1);//底层函数数组数据前移，如：s_ucWitDataBuff[1]的数据移动到s_ucWitDataBuff[0]的位置
		p_WitRegUpdateCbFunc(uiReg2, uiReg2Len);
	}
}

void WitSerialDataIn(uint8_t ucData)//串口1数据输入
{
    uint16_t usCRC16, usTemp, i, usData[4];
    uint8_t ucSum;

    if(p_WitRegUpdateCbFunc == NULL)return ;//判断p_WitRegUpdateCbFunc是否为空，如果不为空就跳过return执行下面的程序
    s_ucWitDataBuff[s_uiWitDataCnt++] = ucData;//串口中断接收到的数据进行赋值到s_ucWitDataBuff数组中
    switch(s_uiProtoclo)//根据前面的模式来选择对应的模式
    {
        case WIT_PROTOCOL_NORMAL:
            if(s_ucWitDataBuff[0] != 0x55)//判断s_ucWitDataBuff数组首位0是否等于0x55，如果不等于执行括号里面的内容，如果等于跳到下一个判断
            {
                s_uiWitDataCnt--;//数据自减
                memcpy(s_ucWitDataBuff, &s_ucWitDataBuff[1], s_uiWitDataCnt);//底层函数数组数据前移，如：s_ucWitDataBuff[1]的数据移动到s_ucWitDataBuff[0]的位置
                return ;
            }
            if(s_uiWitDataCnt >= 11)//判断数据个数是否大于或等于11个，如果是就执行括号里面的内容
            {
                ucSum = __CaliSum(s_ucWitDataBuff, 10);//数据校验和的计算和赋值
                if(ucSum != s_ucWitDataBuff[10])//判断校验和是否和接收到数据的第11个数据是否一样
                {
                    s_uiWitDataCnt--;//数据自减
                    memcpy(s_ucWitDataBuff, &s_ucWitDataBuff[1], s_uiWitDataCnt);//底层函数数组数据前移，如：s_ucWitDataBuff[1]的数据移动到s_ucWitDataBuff[0]的位置
                    return ;
                }
                usData[0] = ((uint16_t)s_ucWitDataBuff[3] << 8) | (uint16_t)s_ucWitDataBuff[2];//计算赋值到usData[0]，s_ucWitDataBuff[3]数据左移8位后与s_ucWitDataBuff[2]进行或运算
                usData[1] = ((uint16_t)s_ucWitDataBuff[5] << 8) | (uint16_t)s_ucWitDataBuff[4];//计算赋值到usData[1]
                usData[2] = ((uint16_t)s_ucWitDataBuff[7] << 8) | (uint16_t)s_ucWitDataBuff[6];//计算赋值到usData[2]
                usData[3] = ((uint16_t)s_ucWitDataBuff[9] << 8) | (uint16_t)s_ucWitDataBuff[8];//计算赋值到usData[3]
                CopeWitData(s_ucWitDataBuff[1], usData, 4);//带入数据到CopeWitData函数中
                s_uiWitDataCnt = 0;
            }
        break;
...
void WitCanDataIn(uint8_t ucData[8], uint8_t ucLen)//CAN数据输入
{
	uint16_t usData[3];
...
int32_t WitRegisterCallBack(RegUpdateCb update_func)//注册获取传感器数据回调函数
{
    if(!update_func)return WIT_HAL_INVAL;//判断update_func是否有数据传入，有就跳过return执行下一步
    p_WitRegUpdateCbFunc = update_func;//把update_func的数据赋值给p_WitRegUpdateCbFunc
    return WIT_HAL_OK;//返回WIT_HAL_OK
}
int32_t WitWriteReg(uint32_t uiReg, uint16_t usData)//服务器写入
{
    uint16_t usCRC;
    uint8_t ucBuff[8];
    if(uiReg >= REGSIZE)return WIT_HAL_INVAL;//判断输入的值是否大于或等于REGSIZE，如果大于就执行return没有就跳过执行下面的程序
    switch(s_uiProtoclo)//根据前面的赋值选择对应的协议
    {
        case WIT_PROTOCOL_NORMAL:
            if(p_WitSerialWriteFunc == NULL)return WIT_HAL_EMPTY;//判断p_WitSerialWriteFunc是否为空
            ucBuff[0] = 0xFF;
            ucBuff[1] = 0xAA;
            ucBuff[2] = uiReg & 0xFF;
            ucBuff[3] = usData & 0xff;
            ucBuff[4] = usData >> 8;
            p_WitSerialWriteFunc(ucBuff, 5);//把数据代入p_WitSerialWriteFunc函数中
            break;
        case WIT_PROTOCOL_MODBUS:
...
int32_t WitReadReg(uint32_t uiReg, uint32_t uiReadNum)//服务器读
{
    uint16_t usTemp, i;
    uint8_t ucBuff[8];
...
int32_t WitInit(uint32_t uiProtocol, uint8_t ucAddr)//初始化标准协议，设置设备地址
{
	if(uiProtocol > WIT_PROTOCOL_I2C)return WIT_HAL_INVAL;//进行判断是否符合条件判断的，符合就执行return的内容，不符合就跳过return执行下一步
    s_uiProtoclo = uiProtocol;//把uiProtocol的数据赋值s_uiProtoclo
    s_ucAddr = ucAddr;//把ucAddr赋值给s_ucAddr
    s_uiWitDataCnt = 0;//s_uiWitDataCnt数据清零
    return WIT_HAL_OK;//返回WIT_HAL_OK
}
void WitDeInit(void)
{
...
int32_t WitDelayMsRegister(DelaymsCb delayms_func)//注册延时回调函数
{
    if(!delayms_func)return WIT_HAL_INVAL;//判断是否有数据传入进来，如果有数据传入进来就跳过return执行下一步
    p_WitDelaymsFunc = delayms_func;//将delayms_func赋值给p_WitDelaymsFunc
    return WIT_HAL_OK;//返回WIT_HAL_OK
}

char CheckRange(short sTemp,short sMin,short sMax)//检查输入范围
{
    if ((sTemp>=sMin)&&(sTemp<=sMax)) return 1;//判断需要两个都成立才会执行return 1，通过判断输入的值都符合两个条件
    else return 0;
}
/*Acceleration calibration demo*/
int32_t WitStartAccCali(void)//开始加速度校准
{
/*
	First place the equipment horizontally, and then perform the following operations
*/
	if(WitWriteReg(KEY, KEY_UNLOCK) != WIT_HAL_OK)	    return  WIT_HAL_ERROR;// unlock reg，判断WitWriteReg结果是否等于WIT_HAL_OK，如果等于跳过return执行下一步
	
	if(s_uiProtoclo == WIT_PROTOCOL_MODBUS)	p_WitDelaymsFunc(20);
	else if(s_uiProtoclo == WIT_PROTOCOL_NORMAL) p_WitDelaymsFunc(1);//判断前面输入的协议类型
	else ;
...
/*change Band*/
int32_t WitSetUartBaud(int32_t uiBaudIndex) //波特率增加或减少
{
...
/*change Bandwidth*/
int32_t WitSetBandwidth(int32_t uiBaudWidth)//带宽缩减/增加
{	
	if(!CheckRange(uiBaudWidth,BANDWIDTH_256HZ,BANDWIDTH_5HZ))//判断是否为1，如果为1不执行括号下面的内容，如果为1会跳过括号的内容执行下一步
	{
		return WIT_HAL_INVAL;
	}
	if(WitWriteReg(KEY, KEY_UNLOCK) != WIT_HAL_OK)	return  WIT_HAL_ERROR;
...
{	
	if(!CheckRange(uiRate,RRATE_02HZ,RRATE_NONE))//返回率增加或减少
	{
		return WIT_HAL_INVAL;
	}
	if(WitWriteReg(KEY, KEY_UNLOCK) != WIT_HAL_OK)	return  WIT_HAL_ERROR;
...
int32_t WitSetContent(int32_t uiRsw) //返回内容
{	
