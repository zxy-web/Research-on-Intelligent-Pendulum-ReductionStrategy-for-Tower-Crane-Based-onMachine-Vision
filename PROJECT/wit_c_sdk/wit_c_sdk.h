...
/* serial function */
typedef void (*SerialWrite)(uint8_t *p_ucData, uint32_t uiLen);//定义一个新的数据类型名称，函数指针
int32_t WitSerialWriteRegister(SerialWrite write_func);//函数指针调用
...
/* Delayms function */
typedef void (*DelaymsCb)(uint16_t ucMs);//定义一个新的数据类型名称，函数指针
int32_t WitDelayMsRegister(DelaymsCb delayms_func);//函数指针的调用


void WitCanDataIn(uint8_t ucData[8], uint8_t ucLen);


typedef void (*RegUpdateCb)(uint32_t uiReg, uint32_t uiRegNum);//定义一个新的数据类型名称，函数指针
int32_t WitRegisterCallBack(RegUpdateCb update_func);//函数指针的调用
