...
void Double_Roll_Pitch_PID_Calculation_z(PIDDouble* axis, float set_point_angle, float angle/*BNO080 旋转角度*/, float rate/*ICM-20602 角速度*/)
{
...
void Double_Roll_Pitch_PID_Calculation(PIDDouble* axis, float set_point_angle, float angle/*BNO080 旋转角度*/, float rate/*ICM-20602 角速度*/)
{
	/*********** Double PID Outer Begin (滚动和俯仰角度位置控制) *************/
	axis->out.reference = set_point_angle;	//Set point of outer PID control外部PID控制量
	axis->out.meas_value = angle;			//BNO080 rotation angle旋转角度
...
//	if(axis->out.error<10 && axis->out.error>-10){
