...
class MainWindow(QMainWindow):
    ...
    def toggle_logging(self):
        if not self.logging_active:
            ...
            self.log_file = open(f"motion_log_{mode}_traj{traj_num}_750g_fil0.4m_{timestamp}.csv", "w")
    ...
    def save_open_loop_trajectory(self):
        ...
        timestamp = time.strftime("%Y%m%d_%H%M%S")
        traj_num = self.selected_trajectory + 1
        filename = f"trajectories/open_loop_traj{traj_num}_750g_0.4m_{timestamp}.csv"
