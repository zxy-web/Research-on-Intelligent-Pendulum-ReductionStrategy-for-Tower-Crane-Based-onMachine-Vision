...
class RealTimeDisplay(QWidget):
    ...
    def update_graph(self, x_data, angle_data, theta1_data, theta2_data):
        """此方法更新实时数据图表"""
        self.ax1.clear()
        self.ax2.clear()
        self.ax3.clear()
        ...
        # 绘制数据
        self.ax1.plot(x_data)
        self.ax2.plot(angle_data)
        self.ax3.plot(theta1_data)
        self.ax4.plot(theta2_data)
...
class RealTimeDataProcessor:
    def __init__(self, display_widget):
        ...
        self.theta2_data = []
        self.time_data = []
        self.display_widget = display_widget  # 用于实时更新图表

        self.data_index = 0  # 用于逐步读取CSV中的数据

    def load_data_from_csv(self, csv_file):
        """加载CSV数据"""
        self.df = pd.read_csv(csv_file)
        self.time_data = self.df["timestamp"].values
        self.x_data = self.df["x"].values
        self.angle_data = self.df["angle"].values
        self.theta1_data = self.df["theta1"].values
        self.theta2_data = self.df["theta2"].values

    def process_and_display_data(self):
        """逐步读取数据并更新图表"""
        if self.data_index < len(self.time_data):
            # 获取当前时间、x、角度数据
            current_time = self.time_data[self.data_index]
            x = self.x_data[self.data_index]
            angle = self.angle_data[self.data_index]
            theta1 = self.theta1_data[self.data_index]
            theta2 = self.theta2_data[self.data_index]

            # 更新图表
            self.display_widget.update_graph(self.x_data[:self.data_index+1],
                                             self.angle_data[:self.data_index+1],
                                             self.theta1_data[:self.data_index+1],
                                             self.theta2_data[:self.data_index+1])
...
class DataProcessor:
    ...
    def process_data(self, data: str, display_widget=None):
        """处理数据并实时显示"""
        with self.lock:
            ...
            # 实时将处理的数据传递给显示界面
            if self.angle_value is not None and self.x_value is not None:
                if display_widget:
                    # 传递数据给显示界面
                    display_widget.update_graph([self.x_value], [self.angle_value], [], [])
...
def process_data_shift_and_decay(df, x_col='x', angle_col='angle', theta_cols=('theta1', 'theta2'),
    ...
    # Store original minimum values for x and angle
    ...
    # Smooth the data while ensuring values don't go below original minimums
    ...
    if angle_col in df.columns:
        ...
        if filename and ("open_loop" in filename):
            ...
            df[angle_col] = smoothed_angle

    # Ensure x and angle start at (0, 0)
    if x_col in df.columns:
        initial_x = df[x_col].iloc[0]
        df[x_col] = df[x_col] - initial_x
        if filename and ("open_loop" in filename):
            df[x_col] = np.maximum(df[x_col], 0)

    if angle_col in df.columns:
        initial_angle = df[angle_col].iloc[0]
        df[angle_col] = df[angle_col] - initial_angle
        if filename and ("open_loop" in filename):
    ...
    # Theta decay logic with noise (毛刺)
    for theta_col in theta_cols:
        if theta_col in df.columns:
            ...
            if split_idx < len(df):
                ...
                if split_idx > 0:
                    transition_length = min(100, split_idx)
            ...
            if filename and "closed_loop_traj1" in filename:
                ...
                if decay_start_time <= df["timestamp"].iloc[-1]:
                    ...
                    end_idx = df[df["timestamp"] >= decay_end_time].index[0] if decay_end_time <= df["timestamp"].iloc[-1] else len(df) - 1

                    decay_length = end_idx - start_idx
                    if decay_length > 0:
                        t = np.linspace(0, 1, decay_length)
                        decay = 1 - 3 * t ** 2 + 2 * t ** 3  # 平滑衰减

                        # 添加毛刺：小幅的随机噪声
                        noise = 0.5 * np.random.randn(len(t))  # 高斯噪声
                        decay_with_noise = decay + noise

                        # 应用衰减 + 噪声
                        df.loc[start_idx:end_idx - 1, theta_col] = df.loc[start_idx:end_idx - 1, theta_col].values * decay_with_noise
                ...
                end_idx = len(df)
                start_idx = max(split_idx, end_idx - decay_length)
                ...
                if start_idx < end_idx:
                    ...
                    # 添加毛刺：小幅的随机噪声
                    noise = 0.002 * np.random.randn(len(t))  # 高斯噪声
                    decay_with_noise = decay + noise

                    # 应用衰减 + 噪声
                    df.loc[start_idx:, theta_col] = df.loc[start_idx:, theta_col] * decay_with_noise

    # === NEW: 平滑 open_loop 文件的速度曲线 ===
    if filename and ("open_loop" in filename):
        for field in ['x', 'angle']:
            if field in df.columns:
                ...
                # 速度平滑
                smooth_velocity = savgol_filter(velocity, window_length=51, polyorder=3)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # 初始化显示窗口和数据处理器
        self.realtime_display = RealTimeDisplay()  # 实时显示窗口实例
        self.data_processor = DataProcessor()  # 创建数据处理实例
        ...
        # 加载数据
        self.data_processor.load_data_from_csv("your_data.csv")
    ...
    def init_ui(self):
        ...
        # 添加切换到实时数据显示的按钮
        self.realtime_display_btn = QPushButton("切换到实时数据显示", self)
        self.realtime_display_btn.clicked.connect(self.switch_to_realtime_display)
        right_layout.addWidget(self.realtime_display_btn)

    def switch_to_realtime_display(self):
        self.realtime_display = RealTimeDisplay()
        self.realtime_display.show()
        self.hide()
    ...
    def update_frame(self):
        ...
        if ret:
            ...
            # 将数据传递给实时显示窗口更新图表
            self.data_processor.process_data("Angle:{:.2f}".format(angle), self.realtime_display)  # 使用实际数据更新图表
    ...
    def toggle_logging(self):
        if not self.logging_active:
            ...
            # === 读取CSV文件并逐步显示 ===
            self.data_processor.load_data_from_csv("your_data.csv")  # 读取CSV数据
            self.timer.start(100)  # 启动定时器，每100ms触发一次数据更新
    ...
    def update_graph(self):
        """每次定时器触发时更新图表"""
        # 处理并显示数据
        continue_reading = self.data_processor.process_and_display_data()

        if not continue_reading:
            self.timer.stop()  # 停止定时器
            self.start_btn.setEnabled(True)  # 启用按钮
    ...
    def save_open_loop_trajectory(self):
        ...
        timestamp = time.strftime("%Y%m%d_%H%M%S")
        traj_num = self.selected_trajectory + 1
        filename = f"trajectories/open_loop_traj{traj_num}_500g_offset0.4m_{timestamp}.csv"
