...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.control_timer.timeout.connect(self.auto_stop_control)
        self.control_timer.setSingleShot(True)  # 仅触发一次，避免重复触发
    ...
    def send_manual_command(self, command):
        ...
        if self.serial_thread and self.serial_thread.isRunning():
            ...
            try:
                ...
                # 如果是上升或下降，启动定时器，10秒后自动停止
                if command == 'up' or command == 'down':
                    self.control_timer.start(10000)  # 10秒后自动停止
                    self.log_message(f"手动控制：{command}已启动，10秒后自动停止")
    ...
    def auto_stop_control(self):
        """定时自动停止控制"""
        self.send_manual_command('stop')  # 自动发送停止指令
        self.log_message("自动停止手动控制操作")
    ...
    def apply_control_logic(self, angle, x_value, vision_data):
        ...
        if not hasattr(self, 'trajectory_generated') or not self.trajectory_generated:
            ...
            if not self.open_loop_mode and self.selected_trajectory == 4:
                self.control_phase = "rising"
                self.phase_start_time = time.time()
                self.send_manual_command('up')
                self.log_message("闭环轨迹5模式：开始上升阶段(10秒)")
                return

        # 新增：处理闭环轨迹5的不同阶段
        if not self.open_loop_mode and self.selected_trajectory == 4:
            elapsed_phase_time = time.time() - self.phase_start_time

            if self.control_phase == "rising" and elapsed_phase_time >= 10.0:
                self.control_phase = "auto_control"
                self.phase_start_time = time.time()
                self.send_manual_command('stop')
                self.log_message("闭环轨迹5模式：上升阶段结束，开始自动控制阶段")

            elif self.control_phase == "auto_control" and elapsed_phase_time >= 30.0:  # 假设自动控制阶段30秒
                self.control_phase = "descending"
                self.phase_start_time = time.time()
                self.send_manual_command('down')
                self.log_message("闭环轨迹5模式：自动控制阶段结束，开始下降阶段(10秒)")
                return

            elif self.control_phase == "descending" and elapsed_phase_time >= 10.0:
                self.control_phase = "finished"
                self.send_manual_command('stop')
                self.log_message("闭环轨迹5模式：下降阶段结束，控制完成")
                self.toggle_control()  # 自动停止控制
                return

            # 如果在上升或下降阶段，不执行自动控制逻辑
            if self.control_phase in ["rising", "descending"]:
                return
