        ...
        for box, cls, conf in zip(boxes, classes, confidences):
            ...
            if self.stabilized:
                with self.lock:
                    ...
                    right_side = dx * 0.0360 / 22.5
                    n_rad = math.atan(right_side)
                    left_side = dy * 0.0360 / 22.5
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.vision = VisionSystem(
            r"D:\桌面\ultralytics-main\ultralytics-main\ultralytics\runs\train\exp4\weights\best.pt")
    ...
    def apply_control_logic(self, angle, x_value, vision_data):
        ...
        if self.open_loop_mode:
            ...
            # === 自适应摆角增益部分 ===
            adapt_factor = 30.0  # 自适应比例，自己可以调，比如20-50效果都行
            K_theta1_current = self.K_theta1 * (1 + adapt_factor * abs(theta1))
            K_theta2_current = self.K_theta2 * (1 + adapt_factor * abs(theta2))

            # === 计算控制量 ===
            pwm_x = (self.Kp_x * err_x +
                     self.Ki_x * self.integral_x -
                     self.Kd_x * d_x +
                     K_theta1_current * theta1)

            pwm_phi = (self.Kp_phi * err_phi +
                       self.Ki_phi * self.integral_phi -
                       self.Kd_phi * d_angle +
                       K_theta2_current * theta2)
    ...
    def toggle_logging(self):
        if not self.logging_active:
            ...
            self.log_file = open(f"motion_log_{mode}_traj{traj_num}_500g_{timestamp}.csv", "w")
            ...
            self.logging_timer.start(50000)
        else:
            if self.log_file:
                self.log_file.close()
            self.logging_active = False
    ...
    def save_open_loop_trajectory(self):
        ...
        timestamp = time.strftime("%Y%m%d_%H%M%S")
        traj_num = self.selected_trajectory + 1
        filename = f"trajectories/open_loop_traj{traj_num}_500g_{timestamp}.csv"
